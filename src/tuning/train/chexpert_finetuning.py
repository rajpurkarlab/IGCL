...
def load_chexpert(cxr_labels, batch_size, base_path=None, pretrained=True, train_percent: Optional[float] = 0.01, K: Optional[int] = None):
    if pretrained:
    ...
    if base_path is None:
        base_path = "/deep/group/data/med-data"

    chexpert_test = CXRTestDataset(f'{base_path}/test_cxr.h5',
                                   f'{base_path}/final_paths.csv',
                                    cxr_labels,
                                    transform,
                                  )
    chexpert_train = CXRTestDataset(f'{base_path}/train.h5',
                                    f'{base_path}/train.csv',
                                    cxr_labels,
                                    transform,
                                   )
    try:
        chexpert_val = CXRTestDataset(f'{base_path}/valid.h5',
