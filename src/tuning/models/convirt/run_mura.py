"""
Train a model for the MURA task.
"""
import os
import argparse
...
from data.loader import MURADataset
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='dataset/mura/', help="Directory to load image data from.")
    parser.add_argument('--local_img_dir', type=str, default='zyh/mura/', help="Directory to load image data from.")
    ...
    parser.add_argument('--root_dir', type=str, default='saved_models/mura', help="Root directory for model saving.")
...
def train(opt):
    logger.info("Training the MURA model...")
    ...
    train_img_file = os.path.join(opt['data_dir'], 'train.txt')
    train_label_file = os.path.join(opt['data_dir'], 'train_labels.csv')
    val_img_file = os.path.join(opt['data_dir'], 'valid.txt')
    val_label_file = os.path.join(opt['data_dir'], 'valid_labels.csv')
    opt['num_class'] = 2

    train_set = MURADataset(
        img_dir=img_dir,
    ...
    while True:
        for i, data in enumerate(train_loader):
            ...
            if step % steps_per_epoch == 0:
                ...
                y_group = val_set.study_ids
                group2label = val_set.study2label
                val_metrics = trainer.get_metrics_by_coalesce(y_pred, y_group, group2label)
                val_score = val_metrics['auc']
                progress_tracker.update_metrics(val_metrics, current_epoch)

                logger.info("| validation @ {:3d}/{:3d} epochs | loss {:.4f} | auc {:.4f}".format(
                    current_epoch,
                    opt['max_epochs'],
                    val_loss,
                    val_metrics['auc']
...
def eval(opt):
    logger.info("Evaluating the MURA model...")
    ...
    test_img_file = os.path.join(opt['data_dir'], 'test.txt')
    test_label_file = os.path.join(opt['data_dir'], 'test_labels.csv')
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader):
            ...
            y_pred += probs
    y_group = test_set.study_ids
    group2label = test_set.study2label
    metrics = trainer.get_metrics_by_coalesce(y_pred, y_group, group2label)
    out_str = "AUC\n" + "{:.2f}".format(
        metrics['auc']*100
    )
