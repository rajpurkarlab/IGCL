...
# Intended to be used with a Global Attention Architecture

class AddMetaNodes(BaseTransform):
    ...
    def __call__(self, data):
        ...
        def make_connection(node_a, node_b):
            ...
            # Add reverse connection if undirected. Actually just add reverse connection regardless
            # if undirected_bool:
            # Add connection
        ...
        # Densely connect dummy nodes to each other
        for i in range(len(cc)):
            for j in range(i, len(cc)):
                dummy_index_a = first_dummy_index + i
                dummy_index_b = first_dummy_index + j
                make_connection(dummy_index_a, dummy_index_b)
