...
class SplitConnectedComponents(BaseTransform):
    ...
    def __call__(self, data):
        ...
        for nodes in cc:
            ...
            cc_edge_index = torch.index_select(data.edge_index, 1,
                                               torch.IntTensor(edges).to(data.edge_index.device))
            cc_edge_attr = torch.index_select(data.edge_attr, 0,
                                              torch.IntTensor(edges).to(data.edge_attr.device))
