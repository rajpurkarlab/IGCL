...
__MODELS = {"resnet18_mimic-cxr":"tuning/pretrained/medaug_mimic-cxr_resnet18.pth.tar","resnet18_chexpert":"tuning/pretrained/medaug_chexpert_resnet18.pth.tar","resnet50_mimic-cxr":"tuning/pretrained/medaug_mimic-cxr_resnet50.pth.tar","resnet50_chexpert":"tuning/pretrained/medaug_chexpert_resnet50.pth.tar"}
...
def _medaug(model="resnet50",pretrained_on="mimic-cxr",device=torch.device("cpu"),freeze_backbone=True,num_out=1):
    ...
    assert pretrained_on in ["mimic-cxr","chexpert"],"Supported model value functions are mimic-cxr and chexpert only."
    ...
    assert isinstance(device,torch.device), "device has to be a type torch.device, use torch.device('"'cuda:0'"' if torch.cuda.is_available() else '"'cpu'"') before calling medaug function"
