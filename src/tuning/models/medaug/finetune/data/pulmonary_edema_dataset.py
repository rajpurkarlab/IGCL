...
class PULMONARY_EDEMADataset(BaseDataset):
    def __init__(self, csv_name, is_training, study_level,
        ...
        if self.is_test_dataset:
            self.csv_path  = PULMONARY_EDEMA_DATA_DIR / self.csv_name  # Modify this
        else:
            self.csv_path =  PULMONARY_EDEMA_DATA_DIR / self.csv_name
    ...
    def get_paths(self, df):
        return df[PULMONARY_EDEMA_COLPATH]
    ...
    def get_labels(self, df):
        # Get the labels
        labels = df[PULMONARY_EDEMA_TASKS]
        return labels
