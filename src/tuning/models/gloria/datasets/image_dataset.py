...
class RanzcrImageDataset(ImageBaseDataset):
    def __init__(self, cfg, split="train", transform=None):

        if RANZCR_DATA_DIR is None:
            raise RuntimeError(
                "RANZCR data path empty\n"
                + "Make sure to download data "
                + f" and update RANZCR_DATA_DIR in ./gloria/constants.py"
            )
        ...
        # read in csv file
        if split == "train":
            self.df = pd.read_csv(RANZCR_TRAIN_CSV)
        elif split == "valid":
            self.df = pd.read_csv(RANZCR_VALID_CSV)
        else:
            self.df = pd.read_csv(RANZCR_TEST_CSV)
        ...
        # get path
        self.df[RANZCR_PATH_COL] = self.df[RANZCR_PATH_COL].apply(
            lambda x: os.path.join(RANZCR_DATA_DIR, "/".join(x.split("/")[1:]))
        )
        ...
        # # replace uncertains
        # uncertain_mask = {k: -1 for k in RANZCR_COMPETITION_TASKS}
        # self.df = self.df.replace(uncertain_mask, CHEXPERT_UNCERTAIN_MAPPINGS)
        super(RanzcrImageDataset, self).__init__(cfg, split, transform)
    ...
    def __getitem__(self, index):
        ...
        # get image
        img_path = row["StudyInstanceUID"]
        x = self.read_from_jpg(img_path)

        # get labels
        y = list(row[RANZCR_TASKS])
        y = torch.tensor(y)
