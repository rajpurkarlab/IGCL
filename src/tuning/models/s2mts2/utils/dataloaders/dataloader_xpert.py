...
class ChexpertDataset(Dataset):
    def __init__(self, root_path, img_filepath, transform, policy='ones'):
        ...
        csv_df = pd.read_csv(img_filepath)

        csv_df = csv_df.sample(64000)

        self.img_paths = list(csv_df["file_path"])
    ...
    def __getitem__(self, item):
        ...
        # print(img)
        img = io.imread(self.img_paths[item])
...
class ChexpertLoader:
    ...
    def run(self, mode='train', ratio=100):
        ...
        if mode == 'unlabel':
            ...
            file_path = 'processed_pretrain_w_lat_count.csv'
        ...
        loader = DataLoader(dataset=chexpert_dataset, batch_size=self.batch_size,
                            shuffle=True if mode !='test'  else False, pin_memory=False,
