    ...
    def transform(self, img):
        ...
        if transform_args.normalization == 'imagenet':
            ...
            normalize = t.Normalize(mean=CHEXPERT_MEAN,
                                    std=CHEXPERT_STD)
        elif transform_args.normalization == 'ranzcr_norm':
            normalize = t.Normalize(mean=RANZCR_MEAN, std = RANZCR_STD)
        elif transform_args.normalization == 'siim_acr_pneumothorax_norm':
            normalize = t.Normalize(mean=SIIM_MEAN, std = SIIM_STD)
        elif transform_args.normalization == 'shenzhen_tb_norm':
            normalize = t.Normalize(mean=SHENZHEN_MEAN, std = SHENZHEN_STD)
        elif transform_args.normalization == 'pediatric_pneumonia_norm':
            normalize = t.Normalize(mean=PEDIATRIC_MEAN, std = PEDIATRIC_STD)
        elif transform_args.normalization == 'pulmonary_edema_norm':
            normalize = t.Normalize(mean=PULMONARY_EDEMA_MEAN, std=PULMONARY_EDEMA_STD)
        elif transform_args.normalization == 'indiana_norm':
            normalize = t.Normalize(mean=INDIANA_MEAN, std=INDIANA_STD)
        elif transform_args.normalization == 'jsrt_norm':
            normalize = t.Normalize(mean=JSRT_MEAN, std=JSRT_STD)
        elif transform_args.normalization == 'montgomery_norm':
            normalize = t.Normalize(mean=MONTGOMERY_MEAN, std=MONTGOMERY_STD)
